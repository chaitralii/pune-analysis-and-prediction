...
import pandas as pd
import base64
from forecast import show_cases
# import forecast
...
def run():
    st.markdown("<h3 style='text-align: left; color: #F63366;'>Let's Predict!</h3>",
                unsafe_allow_html=True)

    date = st.text_input('Enter todays date, to predict confirmed cases for next 7 days!')
    print(date)
    # st.button('search')
    if st.button('search'):
        show_cases()
    ...
    st.title("Covid-19 Vaccination Slots💉")

    st.markdown("<h3 style='text-align: left; color: #F63366;'>Made with Love❤, Stay Negative!</h3>",
                unsafe_allow_html=True)
