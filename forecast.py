...
def show_cases():
    ...
    future_predict = sarimax_mod.predict(start=695, end=695 +10)
    f_temp = pd.DataFrame()
    f_temp['values'] = future_predict.values
    st.write((f_temp['values'].tail(7).astype(int)))
    ...
    for i in range(0,7):
        date_data=((datetime.date.today() + datetime.timedelta(i)).isoformat())
        # st.write(date_data)

    # f_temp = pd.DataFrame()
    # f_temp['date_data'] = str(date_data)
    fig_line1 = px.line(x=date, y=f_temp['values'].tail(7).astype(int),width=700, height=450)
